#
# Code is almost entirely adopted from https://github.com/DamnedFacts/actuonix-lac
#
import struct
import usb.core
import time
...
class LAC:
    ...
    def send_data(self, function, value=0):
        ...
        #response = self.device.read(0x81, 3, 1000)  # 3 because there's three bytes to a packet
        #return (response[2] << 8) + response[1]  # High byte moved left, then tack on the low byte
        #return 100

        # Take data and send it to LAC
    def get_data(self, function, value=0):
        if value < 0 or value > 1023:
            raise ValueError("Value is OOB. Must be 2-byte integer in rage [0, 1023]")
    ...
    def get_feedback(self):
        return self.get_data(self.GET_FEEDBACK)
